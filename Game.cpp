...
// You *must* revise this function according to the RME and spec
bool Game::isValidPickupList
(const string& pickupList, const int pickupFloorNum) const {
    for (int q = 0; q < pickupList.length() - 1; q++) {
        for (int o = q + 1; o < pickupList.length(); o++) {
            if (pickupList.at(q) == pickupList.at(o)) {
                return false;
            }
        }
    }

    for (int q = 0; q < pickupList.length(); q++) {
        if (pickupList.at(q) - '0' < 0 || pickupList.at(q) - '0' > 9) {
            return false;
        }
    }
    ...
    if (building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList.at(0) - '0').getTargetFloor() > building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList.at(0) - '0').getCurrentFloor()) {
        for (int q = 1; q < pickupList.length(); q++) {
            if (building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList.at(q) - '0').getTargetFloor()
                < building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList.at(q) - '0').getCurrentFloor()) {
                return false;
            }
        }
    }
